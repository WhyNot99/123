...
BOOL CTalk915LuckyDrawProDlg::OnInitDialog()
{
	...
	int nNameW = rcClient.Width() * 0.66;
...
{
	//获取全局类指针
	CTalk915LuckyDrawProApp* globe_ptr = (CTalk915LuckyDrawProApp*)AfxGetApp();
	//如果LuckMan.size() 与 RandNum 相同则直接退出
	if (m_listNames.size() == globe_ptr->LuckMan.size())
	{
		CString str = L"已全部抽取";
		m_strLuckyMan = str;
		return;
	...
	while (true)
	{
		int numName = m_listNames.size();
		int RandNum = rand()%numName;
		list<CString>::iterator iter = m_listNames.begin();
		for (int i=0;i<RandNum;i++)
		{
			iter++;
		}
		PostMessage(WM_DISPLAYNAME, (WPARAM)(*iter).AllocSysString());
		if (!m_bRoll)
		{
			OutputLog(L"luck.log",*iter);
			m_strLuckyMan = *iter;
			return;
		}
		Sleep(67);
